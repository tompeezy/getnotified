"use client"
const landlord = [
  {name:"Property Guardian",price:"$599",badge:"Most Popular",desc:"Prevent the worst",features:["3 leak sensors","Automatic water shutoff valve","2 door sensors","1 temp sensor (crawlspace)","Instant alerts to your phone","One-time fee â€” no subscription"]},
...
const elder = [
  {name:"Safe Nights Essentials",price:"$1,499",badge:"Start Here",desc:"Perfect for getting started",features:["Hub computer + Zigbee dongle","2 smart plugs (mesh range)","3 door/window sensors","1 motion sensor","Phone alerts to entire family","One-time fee â€” no subscription"]},
...
const family = [
  {name:"Family Starter",price:"$499",badge:"New Parents",desc:"Home safety for growing families",features:["Hub computer + setup","2 door/window sensors","1 motion sensor","1 water leak sensor","Phone alerts â€” whole family","Works with existing baby monitors"]},
  {name:"Family Shield",price:"$799",badge:"Full Coverage",desc:"Protection room by room",features:["Everything in Starter","4 door/window sensors","2 motion sensors","Smoke detector integration","Temp monitoring (nursery)","No cloud â€” 100% private"]},
]
        ...
        <ul className="space-y-3">
          {p.features.map(f=>(
            <li key={f} className="flex items-start gap-2 text-sm text-slate-600">
              <span className="text-green-500 font-bold mt-0.5">âœ“</span>{f}
            </li>
          ))}
        </ul>
      </div>
  ...
  return (
    <section id="packages" className="py-20 px-6 bg-slate-50">
      <div className="max-w-6xl mx-auto">
        ...
        <p className="text-center text-[#f59e0b] font-bold mb-16">âš¡ 50% deposit secures your install date Â· Balance due at completion</p>
        ...
        <div className="mb-16">
          ...
          <div className="mt-6 bg-amber-50 border border-[#f59e0b] rounded-xl p-4 text-sm text-amber-800 text-center font-semibold">
            ðŸ’° Multi-property discounts available: 2â€“4 units 10% off Â· 5â€“9 units 15% off Â· 10+ units 20% + custom contract
          </div>
        </div>
        ...
        <div className="mb-16">
          ...
          <div className="mt-6 bg-blue-50 border border-blue-200 rounded-xl p-4 text-sm text-blue-800 text-center font-semibold">
            ðŸ’Š Add-ons available: Medication Management +$250 Â· Wellness Monitoring +$175 Â· Monthly Guardian Watch +$35/mo
          </div>
        </div>
        ...
        <div className="mb-8">
          <div className="flex items-center gap-3 mb-8">
            ...
            <div>
              ...
              <p className="text-slate-500 text-sm">Whole-home protection for growing families â€” 100% private, no cloud</p>
            </div>
          </div>
        ...
        <p className="text-center text-slate-400 text-sm mt-8">ðŸ¢ Facility & memory care contracts available Â· ðŸ‡ºðŸ‡¸ Veteran discounts â€” ask when requesting quote Â· Custom quotes for unique properties</p>
      </div>
    </section>
  )
}
